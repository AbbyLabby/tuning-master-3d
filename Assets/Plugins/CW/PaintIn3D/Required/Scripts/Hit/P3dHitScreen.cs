...
namespace PaintIn3D
{
    ...
    public class P3dHitScreen : P3dHitScreenBase
    {
        ...
        public P3dPointConnector Connector
        {
            get
            {
                if (connector == null) connector = new P3dPointConnector();
                return connector;
            }
        }
        ...
        [System.NonSerialized] private List<Link> links = new List<Link>();
        ...
        public void PaintRelease()
        {
            // var link = CwInputManager.Link.Find(links, CwInputManager.Fingers[0]);
            //
            // if (link == null)
            // {
            //     link = CwInputManager.Link.Create(ref links, CwInputManager.Fingers[0]);
            // }
            //
            // link.Finger.Pressure = pressure;
            // link.Finger.ScreenPosition = touchPos;
            //
            // link.Finger.
            //
            // PaintRelease(link, true);
        }
            ...
            if (pixelSpacing > 0.0f)
            {
                ...
                for (var i = 0; i <= steps; i++)
                {
                    ...
                    for (var j = 0; j < gaps; j++)
                    {
                        ...
                        RecordAndPaintAt(link, head, link.History[link.History.Count - 1], false, link.Finger.Pressure,
                            link);
        ...
        private void RecordAndPaintAt(Link link, Vector2 screenNew, Vector2 screenOld, bool preview, float pressure,
            object owner)
        {
...
namespace PaintIn3D
{
    ...
    public class P3dHitScreen_Editor : P3dHitScreenBase_Editor
    {
        protected override void OnInspector()
        {
            ...
            tgt.Connector.HitCache.Inspector(tgt.gameObject, point: point, line: line, triangle: triangle,
                coord: coord);
        }
        ...
        protected override void DrawBasic()
        {
            ...
            if (Any(tgts,
                    t => t.Frequency == P3dHitScreen.FrequencyType.PixelInterval ||
                         t.Frequency == P3dHitScreen.FrequencyType.ScaledPixelInterval ||
                         t.Frequency == P3dHitScreen.FrequencyType.TimeInterval))
            {
                BeginIndent();
